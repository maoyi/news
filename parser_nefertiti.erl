%% This module is working on extracting the information from the website after getting connection to the website.
-module(parser_nefertiti).
%% Create the main method 
-export([check_text/2]).

%% Create a method called "check_text" that goes through the whole website and finds the information (name/datum/tid/picture/description) of the event by searching a specific tag. Each of the information domain is checked and saved in one list individually.
...
take_datum1([$<, $d, $d, $>,_H1,_H2,_H3,_H4|T], Event) -> take_datum(T,[], Event);
take_datum1([_H|T], Event) -> take_datum1(T,Event).

%% It splits the value of 'datum' into two parts and re-formated into a right way 
...
%% Due to the duplicated tag issue this method goes to check another tag to locate the 'Tid'  after the first checking 
...
take_description_p([$&,$q,$u,$o,$t,$;|T], List, Event) ->
    take_description_p(T, [34|List], Event);
take_description_p([H|T], List,Event) -> 
